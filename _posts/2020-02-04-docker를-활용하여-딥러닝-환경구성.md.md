...
도커(Docker)를 활용하면 CUDA, CuDNN과 같은 **복잡한 설치환경의 충돌 걱정 없이** 미리 설치된 dependency 셋팅으로 매우 편리하게 딥러닝/머신러닝을 위한 환경을 구성할 수 있습니다. Docker를 활용하여 Kaggle의 검증된 딥러닝/머신러닝 프레임워크를 구성해보고 jupyter notebook을 원격으로 접속할 수 있도록 설정해보도록 하겠습니다.
...
* 소요시간: Docker Pull Image 시간 제외 **약 10분**
* 환경: Ubuntu 18.04 
...
1. Xgboost, Catboost, Lgbm 을 지원합니다.
...
* `-v` 옵션은 volumn 마운트에 관한 옵션입니다
* `-it` 옵션은 interactive한 환경을 제공합니다 (terminal bash 실행 등)
* 저도 실수한 부분인데 옵션을 만드시 Image명 앞쪽에 지정해 주어야 에러가 안납니다
...
## Jupyter Notebook 외부 접속 허용 및 password 지정
...
## Iptime을 사용하고 있다면, Port Forwarding으로 외부접속 허용하기
