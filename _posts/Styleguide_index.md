...
stage: none
group: Style Guide
info: This is a test adaptation of a piece of the Takeoff documentation style guide at https://docs.Takeoff.com/ee/development/documentation/styleguide/
description: 'Writing styles, markup, formatting, and other standards for Takeoff Documentation.'
---

# Documentation Style Guide

This document defines the standards for Takeoff documentation, including grammar, formatting, word use, and more.

For style questions, mention `@doc-style` in an issue or merge request. If you have access to the Takeoff Slack workspace,
use the `#domain-technical documentation` channel.

In addition to this page, the following resources can help you craft and contribute to documentation:

- [Doc contribution guidelines](../index.md)
- [Doc style and consistency testing](../testing.md)
<!-- - [UI text guidelines](https://design.Takeoff.com/content/error-messages/) -->
- [Takeoff Handbook style guidelines](https://about.Takeoff.com/handbook/communication/#writing-style-guidelines)
- [Microsoft Style Guide](https://docs.microsoft.com/en-us/style-guide/welcome/)
- [Google Developer Documentation Style Guide](https://developers.google.com/style)
<!-- - [Recent updates to this guide](https://Takeoff.com/dashboard/merge_requests?scope=all&utf8=%E2%9C%93&state=merged&label_name[]=tw-style&not[label_name][]=docs%3A%3Afix) -->

## Documentation is the single source of truth (SSOT)

The Takeoff documentation is the SSOT for all
information related to Takeoff implementation, usage, and troubleshooting. It evolves
...
Takeoff adds all troubleshooting information to the documentation, no matter how
unlikely a user is to encounter a situation.

Takeoff Support maintains their own
[troubleshooting content](../../../administration/index.md#support-team-docs)
in the Takeoff documentation.
...
At Takeoff, we have not traditionally used topic types. However, we are starting to
...
Takeoff uses these [topic type templates](../structure.md).
...
- When you encounter new information not available in Takeoff documentation (for
...
of Takeoff should not be written directly in a forum or other messaging system,
...
the Technical Writing team. They're available on Slack in `#docs` or in Takeoff by mentioning the
writer for the applicable [DevOps stage](https://about.Takeoff.com/handbook/product/categories/#devops-stages).
...
The [documentation website](https://docs.Takeoff.com) uses Takeoff Kramdown as its
Markdown rendering engine. For a complete Kramdown reference, see the
[Takeoff Markdown Kramdown Guide](https://about.Takeoff.com/handbook/markdown-guide/).

The [`Takeoff-kramdown`](https://Takeoff.com/Takeoff-org/Takeoff_kramdown) Ruby gem
plans to support all [Takeoff Flavored Markdown](../../../user/markdown.md) in the future, which is
all Markdown supported for display in the Takeoff application itself. For now, use
regular Markdown and follow the rules in the linked style guide.

Kramdown-specific markup (for example, `{:.class}`) doesn't render
properly on Takeoff instances under [`/help`](../index.md#Takeoff-help).
...
Takeoff ensures that the Markdown used across all documentation is consistent, as
...
incorrectly in Takeoff. It also fails when a document has
...
- "Change the `needs` keyword in your `.Takeoff-ci.yml`..."
  - `needs` is a parameter, and `.Takeoff-ci.yml` is a file, so both need backticks.
    Additionally, `.Takeoff-ci.yml` without backticks fails markdownlint because it
...
The documentation is separated by top-level audience folders [`user`](https://Takeoff.com/Takeoff-org/Takeoff-foss/tree/master/doc/user),
[`administration`](https://Takeoff.com/Takeoff-org/Takeoff-foss/tree/master/doc/administration),
and [`development`](https://Takeoff.com/Takeoff-org/Takeoff-foss/tree/master/doc/development)
(contributing) folders.

Beyond that, we primarily follow the structure of the Takeoff user interface or
API.

Our goal is to have a clear hierarchical structure with meaningful URLs like
`docs.Takeoff.com/user/project/merge_requests/`. With this pattern, you can
...
|:----------------------|:------------------|
| `doc/user/`           | User related documentation. Anything that can be done in the Takeoff user interface goes here, including usage of the `/admin` interface. |
| `doc/administration/` | Documentation that requires the user to have access to the server where Takeoff is installed. Administrator settings in the Takeoff user interface are under `doc/user/admin_area/`. |
| `doc/api/`            | API-related documentation. |
| `doc/development/`    | Documentation related to the development of Takeoff, whether contributing code or documentation. Related process and style guides should go here. |
| `doc/legal/`          | Legal documents about contributing to Takeoff. |
| `doc/install/`        | Contains instructions for installing Takeoff. |
| `doc/update/`         | Contains instructions for updating Takeoff. |
...
1. The `doc/user/` directory has five main subdirectories: `project/`, `group/`,
   ...
   - `doc/user/admin_area/` should contain all administrator-related
     documentation describing what can be achieved by accessing the Takeoff
...
- When making reference to other Takeoff products and features, link to their
...
- Write in US English with US grammar. (Tested in [`British.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/British.yml).)
- Use [inclusive language](#inclusive-language).

### Trademark

Only use the Takeoff name and trademarks in accordance with
[Takeoff Brand Guidelines](https://about.Takeoff.com/handbook/marketing/corporate-marketing/brand-activation/brand-guidelines/#trademark).
...
Standards for this content are listed in the [Pajamas Design System Content section](https://design.Takeoff.com/content/punctuation/)
...
- *Feature names are typically lowercase*, like those describing actions and
  ...
  - a to-do item (tested in [`ToDo.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/ToDo.yml))
- *Some features are capitalized*, typically nouns naming Takeoff-specific
  ...
  - the Web IDE
  - Geo
  - Takeoff Runner (see [this issue](https://Takeoff.com/Takeoff-org/Takeoff/-/issues/233529) for details)

Document any exceptions in this style guide. If you're not sure, ask a Takeoff
Technical Writer so that they can help decide and document the result.

Do not match the capitalization of terms or phrases on the [Features page](https://about.Takeoff.com/features/)
or [features.yml](https://Takeoff.com/Takeoff-com/www-Takeoff-com/blob/master/data/features.yml)
by default.
...
- Takeoff [product tiers](https://about.Takeoff.com/pricing/). For example,
  Takeoff Free and Takeoff Ultimate. (Tested in [`BadgeCapitalization.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/BadgeCapitalization.yml).)
...
for the entity, which may use non-standard case styles. For example: Takeoff and
npm.
...
- [Gender-specific wording](#avoid-gender-specific-wording).
  (Tested in [`InclusionGender.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/InclusionGender.yml).)
- [Ableist language](#avoid-ableist-language).
  (Tested in [`InclusionAbleism.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/InclusionAbleism.yml).)
- [Cultural sensitivity](#culturally-sensitive-language).
  (Tested in [`InclusionCultural.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/InclusionCultural.yml).)
...
<!-- vale Takeoff.InclusionGender = NO -->
...
<!-- vale Takeoff.InclusionGender = YES -->
...
<!-- vale Takeoff.InclusionAbleism = NO -->
...
<!-- vale Takeoff.InclusionAbleism = YES -->
...
<!-- vale Takeoff.InclusionCultural = NO -->
...
<!-- vale Takeoff.InclusionCultural = YES -->
...
| admin, admin area     | Use **administration**, **administrator**, **administer**, or **Admin Area** instead. ([Vale](../testing.md#vale) rule: [`Admin.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/Admin.yml)) |
...
| currently             | Do not use when talking about the product or its features. The documentation describes the product as it is today. ([Vale](../testing.md#vale) rule: [`CurrentStatus.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/CurrentStatus.yml)) |
| easily                | Do not use. If the user doesn't find the process to be these things, we lose their trust. |
| e.g.                  | Do not use Latin abbreviations. Use **for example**, **such as**, **for instance**, or **like** instead. ([Vale](../testing.md#vale) rule: [`LatinTerms.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/LatinTerms.yml)) |
| future tense          | When possible, use present tense instead. For example, use `after you execute this command, Takeoff displays the result` instead of `after you execute this command, Takeoff will display the result`. ([Vale](../testing.md#vale) rule: [`FutureTense.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/FutureTense.yml)) |
| handy                 | Do not use. If the user doesn't find the process to be these things, we lose their trust. |
| high availability, HA | Do not use. Instead, direct readers to the Takeoff [reference architectures](../../../administration/reference_architectures/index.md) for information about configuring Takeoff for handling greater amounts of users. |
| I                     | Do not use first-person singular. Use **you**, **we**, or **us** instead. ([Vale](../testing.md#vale) rule: [`FirstPerson.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/FirstPerson.yml)) |
| i.e.                  | Do not use Latin abbreviations. Use **that is** instead. ([Vale](../testing.md#vale) rule: [`LatinTerms.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/LatinTerms.yml)) |
...
| me, myself, mine      | Do not use first-person singular. Use **you**, **we**, or **us** instead. ([Vale](../testing.md#vale) rule: [`FirstPerson.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/FirstPerson.yml)) |
...
| profanity             | Do not use. Doing so may negatively affect other users and contributors, which is contrary to the Takeoff value of [Diversity, Inclusion, and Belonging](https://about.Takeoff.com/handbook/values/#diversity-inclusion). |
| scalability           | Do not use when talking about increasing Takeoff performance for additional users. The words scale or scaling are sometimes acceptable, but references to increasing Takeoff performance for additional users should direct readers to the Takeoff [reference architectures](../../../administration/reference_architectures/index.md) page. |
...
| via                   | Do not use Latin abbreviations. Use **with**, **through**, or **by using** instead. ([Vale](../testing.md#vale) rule: [`LatinTerms.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/LatinTerms.yml)) |
...
[user interfaces](https://design.Takeoff.com/content/punctuation/#contractions).
...
| Do not use double spaces. (Tested in [`SentenceSpacing.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/SentenceSpacing.yml).) | --- |
| Do not use tabs for indentation. Use spaces instead. You can configure your code editor to output spaces instead of tabs when pressing the tab key. | --- |
| Use serial commas (_Oxford commas_) before the final _and_ or _or_ in a list of three or more items. (Tested in [`OxfordComma.yml`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/doc/.vale/Takeoff/OxfordComma.yml).) | _You can create new issues, merge requests, and milestones._ |
...
Tested in [`lint-doc.sh`](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/scripts/lint-doc.sh).
...
[Takeoff UX guide](https://design.Takeoff.com/content/punctuation/).
...
- [Avoid using symbols and special characters](https://Takeoff.com/Takeoff-org/Takeoff-docs/-/issues/84)
...
- When introducing a new document, be careful for the headings to be
  grammatically and syntactically correct. Mention an [assigned technical writer (TW)](https://about.Takeoff.com/handbook/product/categories/)
  for review.
...
NOTE:
[Introduced](https://Takeoff.com/Takeoff-org/Takeoff/-/merge_requests/39717) in
Takeoff 13.4, [product badges](#product-tier-badges) used in headings aren't
...
Keep in mind that the Takeoff user interface links to many documentation pages
...
- If possible, avoid changing headings, because they're not only linked internally.
  There are various links to Takeoff documentation on the internet, such as
...
- Use [meaningful anchor texts](https://www.futurehosting.com/blog/links-should-have-meaningful-anchor-text-heres-why/).
  For example, instead of writing something like `Read more about Takeoff Issue Boards [here](LINK)`,
  write `Read more about [Takeoff Issue Boards](LINK)`.
...
- Do not use absolute URLs or URLs from `docs.Takeoff.com`.
...
- Don't link relative to root. For example, `/ee/user/Takeoff_com/index.md`.

  Don't:

  - `https://docs.Takeoff.com/ee/administration/geo/replication/troubleshooting.html`
...
NOTE:
Using the Markdown extension is necessary for the [`/help`](../index.md#Takeoff-help)
section of Takeoff.
...
```markdown
For more information, see the [confidential issue](../../../user/project/issues/confidential_issues.md) `https://Takeoff.com/Takeoff-org/Takeoff-foss/-/issues/<issue_number>`.
```
...
- _Do_: `[link to line 3](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/11f17c56d8b7f0b752562d78a4298a3a95b5ce66/.Takeoff/issue_templates/Feature%20proposal.md#L3)`
- _Don't_: `[link to line 3](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/.Takeoff/issue_templates/Feature%20proposal.md#L3).`
...
Use these terms when referring to the main Takeoff user interface
elements:

- **Top menu**: This is the top menu that spans the width of the user interface.
  It includes the Takeoff logo, search field, counters, and the user's avatar.
...
- _Capture the most relevant area of the page._ Don't include unnecessary white
  ...
  sidebar of the Takeoff user interface can change, so don't include the sidebar
...
- Save the image with a lowercase filename that's descriptive of the feature
  or concept in the image. If the image is of the Takeoff interface, append the
  ...
  page of Takeoff 11.1, a valid name is `pipelines_v11_1.png`. If you're adding an
...
Takeoff has a [Rake task](https://Takeoff.com/Takeoff-org/Takeoff/-/blob/master/lib/tasks/pngquant.rake)
that you can use to automate the process. In the root directory of your local
copy of `https://Takeoff.com/Takeoff-org/Takeoff`, run in a terminal:
...
Adding Takeoff YouTube video tutorials to the documentation is highly
...
> [Introduced](https://Takeoff.com/Takeoff-org/Takeoff-docs/-/merge_requests/472) in Takeoff 12.1.

The [Takeoff documentation site](https://docs.Takeoff.com) supports embedded
videos.

You can embed videos from [the official YouTube account for Takeoff](https://www.youtube.com/channel/UCnMGQ8QHMAnVIsI3xJrihhg) only.
...
This is how it renders on the Takeoff documentation site:

<div class="video-fallback">
  See the video: <a href="https://www.youtube.com/watch?v=enMumwvLAug">What is Takeoff</a>.
</div>
<figure class="video-container">
  <iframe src="https://www.youtube.com/embed/MqL6BMOySIQ" frameborder="0" allowfullscreen="true"> </iframe>
</figure>
...
`/help`, because the Takeoff Markdown processor doesn't support iframes. It's
...
Syntax highlighting is required for fenced code blocks added to the Takeoff
...
For a complete reference on code blocks, see the [Kramdown guide](https://about.Takeoff.com/handbook/markdown-guide/#code-blocks).

## Takeoff SVG icons

> [Introduced](https://Takeoff.com/Takeoff-org/Takeoff-docs/-/issues/384) in Takeoff 12.7.

You can use icons from the [Takeoff SVG library](https://Takeoff-org.Takeoff.io/Takeoff-svgs/)
directly in the documentation.

This way, you can achieve a consistent look when writing about interacting with
Takeoff user interface elements.
...
- Icon with custom size and class: `**{icon-name, size, class-name}**`.
  ...
  You can access any class available to this element in Takeoff documentation CSS.
...
| Section                  | Description                                                                                                                 |
|:-------------------------|:----------------------------------------------------------------------------------------------------------------------------|
| **{overview}** Overview  | View your Takeoff Dashboard, and administer projects, users, groups, jobs, runners, and Gitaly servers.                      |
| **{monitor}** Monitoring | View Takeoff system information, and information on background jobs, logs, health checks, requests profiles, and audit events. |
...
Alert boxes render only on the Takeoff documentation site (<https://docs.Takeoff.com>).
In the Takeoff product help, alert boxes appear as plain text.
...
Takeoff product documentation pages (not including [Contributor and Development](../../README.md)
...
The Takeoff Technical Writing team determines which versions of
documentation to display on this site based on the Takeoff
[Statement of Support](https://about.Takeoff.com/support/statement-of-support.html#we-support-the-current-major-version-and-the-two-previous-major-versions).

### View older Takeoff documentation versions

Older versions of Takeoff may no longer have documentation available from `docs.Takeoff.com`.
If documentation for your version is no longer available from `docs.Takeoff.com`, you can still view a
tagged and released set of documentation for your installed version:

- In the [documentation archives](https://docs.Takeoff.com/archives/).
- At the `/help` URL of your Takeoff instance.
- In the documentation repository based on the respective branch (for example,
  the [13.2 branch](https://Takeoff.com/Takeoff-org/Takeoff/-/tree/13-2-stable-ee/doc)).
...
> [Introduced](<link-to-issue>) in Takeoff 11.3.
...
> - [Introduced](<link-to-issue>) in Takeoff 11.3.
> - [Enabled by default](<link-to-issue>) in Takeoff 11.4.
...
```markdown
> - [Moved](<link-to-issue>) from Takeoff Ultimate to Takeoff Premium in 11.8.
> - [Moved](<link-to-issue>) from Takeoff Premium to Takeoff Free in 12.0.
```

If a feature is deprecated, include a link to a replacement (when available):

```markdown
> - [Deprecated](<link-to-issue>) in Takeoff 11.3. Replaced by [meaningful text](<link-to-appropriate-documentation>).
```
...
```markdown
WARNING:
This feature was [deprecated](link-to-issue) in Takeoff 12.3 and replaced by
[Feature name](link-to-feature-documentation).
```

In the first major Takeoff version after the feature was deprecated, be sure to
...
In this case, add `([introduced/deprecated](<link-to-issue>) in Takeoff X.X)`.

Including the issue link is encouraged, but isn't a requirement. For example:

```markdown
The voting strategy in Takeoff 13.4 and later requires the primary and secondary
voters to agree.
```
...
for use in Takeoff X.X, and is planned for [removal](link-to-issue) in Takeoff X.X.
```

After the feature or product is officially deprecated and removed, remove
its information from the Takeoff documentation.
...
- Available in Takeoff 13.1 and earlier.
- Available in Takeoff 12.4 and later.
- In Takeoff 12.2 and earlier, ...
- In Takeoff 11.6 and later, ...

### Removing versions after each major release

Whenever a major Takeoff release occurs, we remove all version references
to now-unsupported versions of Takeoff. Note that this includes the removal of
specific instructions for users of non-supported Takeoff versions. For example,
if Takeoff versions 11.x and later are supported, special
instructions for users of Takeoff 10 should be removed.

To view historical information about a feature, review Takeoff
[release posts](https://about.Takeoff.com/releases/), or search for the issue or
...
|:--------------------------------------------------------------------------|:----------------------|
| Takeoff Free self-managed and SaaS, and higher tiers                       | `**(FREE)**`          |
| Takeoff Premium self-managed and SaaS, and their higher tiers              | `**(PREMIUM)**`       |
| Takeoff Ultimate self-managed and SaaS                                     | `**(ULTIMATE)**`      |
| _Only_ Takeoff Free self-managed and higher tiers (no SaaS-based tiers)    | `**(FREE SELF)**`     |
| _Only_ Takeoff Premium self-managed and higher tiers (no SaaS-based tiers) | `**(PREMIUM SELF)**`  |
| _Only_ Takeoff Ultimate self-managed (no SaaS-based tiers)                 | `**(ULTIMATE SELF)**` |
| _Only_ Takeoff Free SaaS and higher tiers (no self-managed instances)      | `**(FREE SAAS)**`     |
| _Only_ Takeoff Premium SaaS and higher tiers (no self-managed instances)   | `**(PREMIUM SAAS)**`  |
| _Only_ Takeoff Ultimate SaaS (no self-managed instances)                   | `**(ULTIMATE SAAS)**` |

Topics that mention the `Takeoff.rb` file are referring to
self-managed instances of Takeoff. To prevent confusion, include the relevant `TIER ONLY`
...
When a restart or reconfigure of Takeoff is required, avoid duplication by linking
to [`doc/administration/restart_Takeoff.md`](../../../administration/restart_Takeoff.md)
with text like this, replacing 'reconfigure' with 'restart' as needed:

```markdown
Save the file and [reconfigure Takeoff](../../../administration/restart_Takeoff.md)
for the changes to take effect.
```

If the document resides outside of the `doc/` directory, use the full path
instead of the relative link:
`https://docs.Takeoff.com/ee/administration/restart_Takeoff.html`.
...
Takeoff supports two installation methods: installations from source, and Omnibus
...
Takeoff, or restart Takeoff. Use these styles to document these steps, replacing
`PATH/TO` with the appropriate path:

````markdown
**For Omnibus installations**

1. Edit `/etc/Takeoff/Takeoff.rb`:

   ```ruby
   external_url "https://Takeoff.example.com"
   ```

1. Save the file and [reconfigure](PATH/TO/administration/restart_Takeoff.md#omnibus-Takeoff-reconfigure)
   Takeoff for the changes to take effect.

---

**For installations from source**

1. Edit `config/Takeoff.yml`:

   ```yaml
   Takeoff:
     host: "Takeoff.example.com"
   ```

1. Save the file and [restart](PATH/TO/administration/restart_Takeoff.md#installations-from-source)
   Takeoff for the changes to take effect.
````
...
- Use the [Takeoff Restart](#Takeoff-restart) section to explain any required
  restart or reconfigure of Takeoff.

## Feature flags

Learn how to [document features deployed behind flags](../feature_flags.md). For
guidance on developing Takeoff with feature flags, see [Feature flags in development of Takeoff](../../feature_flags/index.md).
